...
//     std::cout << "[DEBUG] Task " << taskid << ": Total length = " << len << ", task width = " << width << ", blocksize = " << blocksize << ", numblocks = " << blocks << std::endl;
//     LOCK_RELEASE(output_lck);
// #endif
    for(int i = (threadid-leader) * (blocks/width) * blocksize; (i < len) && (i < ((threadid-leader)+1) * (blocks/width) * blocksize); i++){
// #ifdef DEBUG
//       LOCK_ACQUIRE(output_lck);

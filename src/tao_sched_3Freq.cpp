...
int num_task[TASKTYPES][XITAO_MAXTHREADS * XITAO_MAXTHREADS] = {0};
#endif
...
int worker_loop(int nthread){
  ...
  // #endif
          if(current_freq[nthread] < 2035200){
            // For Parallelism = 6, three threshold levels 
            if(worker_ready_q[nthread].size() >= 2){
              ...
              if(worker_ready_q[nthread].size() >= 1 && current_freq[nthread] == 345600){

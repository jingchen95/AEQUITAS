   ...
   for (kk=0; kk<NB; kk++) {
      ...
      if(dependency_matrix[kk][kk]) {
        ...
        dependency_matrix[kk][kk]->make_edge(lu0);
         ...
         // gotao_push(lu0);
         lu0->width = TAO_WIDTH;
         lu0->leader = rand() % gotao_nthreads; //LU0 tasks are randomly executed on Denver
         // if(queue < 2){ // Schedule to Denver
         //    lu0->width = pow(2, rand() % 2); // Width: 1 2
         //    lu0->leader = (rand() % (2/lu0->width)) * lu0->width;
         // }else{ // Schedule to A57
         //    lu0->width = pow(2, rand() % 3); // Width: 1 2 4
         //    lu0->leader = 2 + (rand() % (4/lu0->width)) * lu0->width;
         // }
...
#endif
   if(schedulerid < 3){
      LU0::print_ptt(LU0::time_table, "LU0");
      FWD::print_ptt(FWD::time_table, "FWD");
      BDIV::print_ptt(BDIV::time_table, "BDIV");
      BMOD::print_ptt(BMOD::time_table, "BMOD");
   }
}

  ...
  if (pid == 0){ /* child */
    ...
    while(getppid() == ppid_before_fork){
      ...
      if(i > 2){ // Two Denver cores are both inactive
        ...
        Denver << std::to_string(806400) << std::endl;
#ifdef DEBUG
        LOCK_ACQUIRE(output_lck);
      ...
      if(i > 4){
        ...
        ARM << std::to_string(806400) << std::endl;
#ifdef DEBUG
        LOCK_ACQUIRE(output_lck);
  ...
  // #endif
          ...
          //   }
            ...
            if(worker_ready_q[nthread].size() >= 4*(float(Parallelism)/5.0)){ // For Parallelism = 8, five threshold levels 
              ...
              if(worker_ready_q[nthread].size() >= 3*(float(Parallelism)/5.0) && current_freq[nthread] < 1728000){
                required_freq[nthread] = 1728000;
              }else{
                if(worker_ready_q[nthread].size() >= 2*(float(Parallelism)/5.0) && current_freq[nthread] < 1420800){
                  required_freq[nthread] = 1420800;
                }else{
                  if(worker_ready_q[nthread].size() >= float(Parallelism)/5.0 && current_freq[nthread] < 1113600){
                    required_freq[nthread] = 1113600;
                  }
                }
              }
      ...
      if(task_completions[nthread].tasks > 0){
        ...
        // if(stealings[nthread].begin() >=0){
          ...
          //   }
            ...
            if(current_freq[freq_up_core] == 806400){
              required_freq[freq_up_core] = 1113600;
            }else{
              if(current_freq[freq_up_core] == 1113600){
                required_freq[freq_up_core] = 1420800;

              }else{
                if(current_freq[freq_up_core] == 1420800){
                  required_freq[freq_up_core] = 1728000;
                }else{
                  if(current_freq[freq_up_core] == 1728000){
                    required_freq[freq_up_core] = 2035200;
                  }
                }
              }
          ...
          // if(stealings[freq_up_core].begin() >= 0){
            ...
            if(current_freq[freq_up_next_core] < 2035200){
              ...
              if(current_freq[freq_up_next_core] == 806400){
                required_freq[freq_up_next_core] = 1113600;
              }else{
                if(current_freq[freq_up_next_core] == 1113600){
                  required_freq[freq_up_next_core] = 1420800;

                }else{
                  if(current_freq[freq_up_next_core] == 1420800){
                    required_freq[freq_up_next_core] = 1728000;
                  }else{
                    if(current_freq[freq_up_next_core] == 1728000){
                      required_freq[freq_up_next_core] = 2035200;
                    }
                  }
                }
      ...
      do{
        ...
        if(!worker_ready_q[random_core].empty()){
          ...
          //if((st->criticality == 0 && Sched == 0) || (Sched == 3)){
            ...
            if(current_freq[random_core] == 1728000){
              required_freq[nthread] = 1420800;
            }
            if(current_freq[random_core] == 1420800){
              required_freq[nthread] = 1113600;
            }
            if(current_freq[random_core] == 1113600){
              required_freq[nthread] = 806400;
            }
#ifdef DEBUG
            LOCK_ACQUIRE(output_lck);
